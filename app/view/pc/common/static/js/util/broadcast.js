...
(function(){
    ...
    broadcast.definedChannel = {
        //  name              description
        "WGT_NAV_CHANGE" : "ucenter-widget-nav-change"
    };
    broadcast.prototype = {
        init : function(){
            var _this = this;
            _this.channel();
            //broadcast.definedChannel = broadcast.definedChannel;
        },
         ...
         */
        help : function(type){
            if(_this._option.debug){
                console.info("【broadcast help info：】");
                console.info("listen(name, listenCallback) //name: 广播频道，listenCallback：监听方法");
                console.info("trigger(name, data) //name: 广播频道，data：参数对象");
                console.info("channel() //获取所用频道name");
            }
        }
    };
    
    webos.common.broadcast = new broadcast();
})();
